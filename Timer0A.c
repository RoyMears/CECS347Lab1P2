...
void Timer1A_Init(void(*task)(void), unsigned long period){
  ...
  TIMER1_TAMR_R = 0x00000001;   // 3) oneshot timer mode
	
	//both reload value and prescale need to be modified for 1us
	
  TIMER1_TAILR_R = (period-1);    // 4) reload value
  TIMER1_TAPR_R = 99;            // 5) bus clock prescale 99
			 
  TIMER1_ICR_R = 0x00000001;    // 6) clear TIMER0A timeout flag
  TIMER1_IMR_R = 0x00000001;    // 7) arm timeout interrupt
  NVIC_PRI5_R = (NVIC_PRI5_R&0xFFFF0FFF)|0x0000A000; // 8) clear bits 15-13 and set priority to 5
